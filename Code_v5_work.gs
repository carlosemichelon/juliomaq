...
// ════════════════════════════════════════════════════════════════
// JuliomaqOS v5 — Code.gs
// ORDEM DAS FUNÇÕES (espelha abas do index_v5_work.html):
//
//  1. INFRAESTRUTURA  onOpen, doGet, doPost, getSheet, backup, utils
//  2. AGENDA          salvarAgendaEvento, getAgendaEventosSalvos, editar, apagar
//  3. ATIVIDADES SF   salvarAtividade, getAtividades, editar, apagar, atualizarStatus
//  4. DESPESAS        salvarDespesa, getDespesas, editarDespesa, marcarReembolso
//  5. VEÍCULOS        salvarManutencao, getManutencoes, editarManutencao, semanal
//  6. OS              salvarOrdem, getOrdens, editarOS, marcarPaga, fotos, assinatura
//  7. GARANTIA/SF     getGarantias, atualizarCampoGarantia
//  8. FINANCEIRO      salvarFinanceiro, getFinanceiro
//  9. VENDAS          salvarVenda, getVendas, parcelas, mídias
// 10. ORÇAMENTOS      salvarOrcamento, getOrcamentos
// 11. PROSPECÇÃO      salvarProspeccao, getProspeccoes
// 12. PARQUE MÁQ.     salvarParqueMaquinas, getParqueMaquinas
// 13. FERRAMENTAS     salvarFerramenta, getFerramentas
// 14. VÍNCULOS        vincularOSEvento, desvincularOSEvento, vincularOrcamento
// 15. LER TUDO        lerTudoPlanilha — chamada ao iniciar o app
// 16. SETUP/FÓRMULAS  setupFormulas e helpers
// 17. DIAGNÓSTICO     Funções de manutenção — rodar manualmente no Apps Script
// ════════════════════════════════════════════════════════════════
...
function onOpen(){
  try{
    ...
    // Menu principal com ferramentas de manutenção
    ui.createMenu('⚙️ JuliomaqOS')
      .addItem('🔨 Corrigir fórmulas da planilha', 'corrigirFormulasPlanilha')
      .addItem('📋 Corrigir cabeçalho Atividades', 'corrigirCabecalhoAtividades')
      .addItem('🗂️ Ordenar Agenda por data', 'ordenarAgendaPorData')
      .addSeparator()
      ...
      .addSeparator()
      .addItem('🧪 Testar salvamento', 'testarSalvamento')
      .addItem('📊 Diagnóstico Agenda', 'diagnosticarAgenda')
      .addToUi();
...
// ID fixo da planilha — necessário para doPost funcionar fora do contexto ativo
var SPREADSHEET_ID = '1AMwOpaVa2hcpiaVmXDcZvNiE-7rQeyt4vDU42GkLrLc';
function _getSS(){
...
function doPost(e){
  var WHITELIST = [
    ...
    'salvarGarantia',
    'salvarAtividade','getAtividades','atualizarStatusAtividade','editarAtividade','apagarAtividade','diagnosticarAtividades','corrigirCabecalhoAtividades','criarOTsRetroativas','testarAtividades','diagnosticarIdsAgenda','ordenarAgendaPorData','limparCasoSFErrado',
    ...
    'getAgendaEventos',
    'marcarReembolso','removerAbasEventosLeads','corrigirLinhasDespesas','corrigirLinhasVeiculos','recuperarDadosDespesas','testarSalvamento','escreverLinhaTesteDireto','corrigirFormulasPlanilha',
...
function _criarOTSeNecessario(evId, titulo, cliente, dataISO, tecnicos, descricaoEvento){
  ...
  try{
    ...
    var HDR=['ID','Topico','Descricao','Data','Status','ID_Ref','Observacao','CasoSF','EvId','CriadoEm','DescricaoEvento'];

    if(sheet.getLastRow()===0){
      sheet.appendRow(HDR);
      sheet.getRange(1,1,1,11).setBackground('#00396b').setFontColor('#fff').setFontWeight('bold');
    ...
    sheet.appendRow([id,'OT Pendentes',descricao,fmtD(dataISO),'Solicitado',idRef,'','',evId,new Date().toLocaleString('pt-BR'),descricaoEvento||'']);
...
function salvarAgendaEvento(dados){
  ...
  try{
    ...
    if(dados.id){
      ...
      if(dados.tecnicos) _criarOTSeNecessario(dados.id,dados.titulo||'',dados.cliente||'',dados.dataISO||'',dados.tecnicos||'',dados.descricao||'');
    ...
    SpreadsheetApp.flush();
    _criarOTSeNecessario(id,dados.titulo||'',dados.cliente||'',dados.dataISO||'',dados.tecnicos||'',dados.descricao||'');
...
// Retorna a ultima linha com dado real na coluna A (ignora formulas/vazios abaixo)
function _ultimaLinhaReal(sheet){
  // Acha o fim do bloco continuo de dados a partir da linha 2
  // Para na primeira linha vazia — ignora linhas deslocadas alem da lacuna
  var lastRow=sheet.getLastRow();
  if(lastRow<2) return 1;
  // Le ate no maximo 500 linhas (evita ler linhas deslocadas na 1000+)
  var limite=Math.min(lastRow,500);
  var colA=sheet.getRange(2,1,limite,1).getValues();
  var ultimaCheia=1; // linha do cabecalho
  for(var i=0;i<colA.length;i++){
    var val=String(colA[i][0]||'').trim();
    if(val==='') break; // parou no primeiro vazio — fim do bloco real
    ultimaCheia=i+2; // +2 porque comeca na linha 2
  }
  return ultimaCheia;
}
...
function salvarDespesa(dados){
  try{
    var ID='1AMwOpaVa2hcpiaVmXDcZvNiE-7rQeyt4vDU42GkLrLc';
    var ss=SpreadsheetApp.openById(ID);
    var sheet=ss.getSheetByName('Despesas');
    if(!sheet) sheet=ss.insertSheet('Despesas');

    // Header se vazio
    if(sheet.getLastRow()===0){
      sheet.appendRow(['No','Data','Comprador','CNPJ','Fornecedor','Descricao','Categoria','Valor','FormaPgto','Foto','SalvoEm','Assinatura','FotoVisual','AssinaturaVisual','Reembolsado']);
    }
    ...
    var num='DESP-'+String(maxN+1).padStart(3,'0');

    // Foto no Drive
    var fotoUrl='';
    if(dados.fotoB64&&dados.fotoB64.length>10){
      try{
        var b64=dados.fotoB64.indexOf('data:')===0?dados.fotoB64.split(',')[1]:dados.fotoB64;
      ...
      }catch(fErr){Logger.log('Foto err: '+fErr);}
    }
    ...
    if(dados.assinatura&&dados.assinatura.length>10){
      try{
        ...
        var sigFolder=folders.hasNext()?folders.next():DriveApp.createFolder('Juliomaq_Assinaturas');
        var b64s=dados.assinatura.indexOf('data:')===0?dados.assinatura.split(',')[1]:dados.assinatura;
        var bytesS=Utilities.base64Decode(b64s);
        var blobS=Utilities.newBlob(bytesS,'image/jpeg',num+'_sig.jpg');
        var fileS=sigFolder.createFile(blobS);
        fileS.setSharing(DriveApp.Access.ANYONE_WITH_LINK,DriveApp.Permission.VIEW);
        sigUrl='https://drive.google.com/file/d/'+fileS.getId()+'/view';
      }catch(sErr){Logger.log('Sig err: '+sErr);}
    }

    var newRow=_ultimaLinhaReal(sheet)+1;
    var rowDesp=[num,dados.data||'',dados.comprador||'',dados.cnpj||'',dados.fornecedor||'',dados.descricao||'',dados.categoria||'',parseFloat(dados.valor)||0,dados.formaPgto||'',fotoUrl,new Date().toLocaleString('pt-BR'),sigUrl,'','','Nao'];
    sheet.getRange(newRow,1,1,rowDesp.length).setValues([rowDesp]);

    // Formulas IMAGE separadas
    if(fotoUrl) sheet.getRange(newRow,13).setFormula('=IMAGE("'+fotoUrl+'")');
    if(sigUrl)  sheet.getRange(newRow,14).setFormula('=IMAGE("'+sigUrl+'")');

    SpreadsheetApp.flush();
    Logger.log('salvarDespesa OK: '+num);
    return {success:true,num:num};
  }catch(e){
    Logger.log('salvarDespesa ERRO: '+e.toString());
    return {success:false,error:e.toString()};
  }
}
...
function salvarManutencao(dados){
  try{
    Logger.log('salvarManutencao inicio: '+dados.veiculo+' / '+dados.tipo);
    var ss=_getSS();
    var sheet=ss.getSheetByName('Gestao de Veiculos');
    if(!sheet) sheet=ss.insertSheet('Gestao de Veiculos');

    // Header se vazio
    if(sheet.getLastRow()===0){
      sheet.appendRow(['No','Data','Veiculo','Tipo','Responsavel','Hodometro (km)','Valor R$','Descricao','Assinatura','Salvo em']);
    }

    // Numero sequencial — usa _ultimaLinhaReal para ignorar linhas deslocadas
    var ultimaReal=_ultimaLinhaReal(sheet);
    var maxN=0;
    if(ultimaReal>1){
      var nums=sheet.getRange(2,1,ultimaReal-1,1).getValues();
      nums.forEach(function(r){var m=String(r[0]||'').match(/\d+/);if(m)maxN=Math.max(maxN,parseInt(m[0]));});
    }
    var num='VM-'+String(maxN+1).padStart(3,'0');
    Logger.log('salvarManutencao: num='+num+' ultimaReal='+ultimaReal);

    var sigB64=dados.assinatura&&dados.assinatura.indexOf('data:')===0?dados.assinatura:'';
    if(sigB64.length>400000) sigB64='';

    // Escreve na linha correta — NAO usa appendRow (vai para getLastRow+1)
    var newRow=ultimaReal+1;
    var rowData=[
      num,
      dados.data||'',
      dados.veiculo||'',
    ...
    // Assinatura visual separado
    if(sigB64){
      try{
        var folders=DriveApp.getFoldersByName('Juliomaq_Assinaturas');
        var folder=folders.hasNext()?folders.next():DriveApp.createFolder('Juliomaq_Assinaturas');
        var b64=sigB64.split(',')[1];
        var bytes=Utilities.base64Decode(b64);
        var blob=Utilities.newBlob(bytes,'image/jpeg',num+'_sig.jpg');
        var file=folder.createFile(blob);
        file.setSharing(DriveApp.Access.ANYONE_WITH_LINK,DriveApp.Permission.VIEW);
        var url='https://drive.google.com/file/d/'+file.getId()+'/view';
        sheet.getRange(newRow,11).setFormula('=IMAGE("'+url+'")');
      }catch(sigErr){Logger.log('salvarManutencao sig err: '+sigErr);}
    }

    SpreadsheetApp.flush();
    Logger.log('salvarManutencao SUCESSO: '+num+' linha='+newRow);
    return {success:true, num:num};
  }catch(e){
    Logger.log('salvarManutencao ERRO: '+e.toString());
    return {success:false, error:e.toString()};
  }
}
...
function corrigirCabecalhoAtividades(){
  ...
  try{
    ...
    for(var i=1;i<allData.length;i++){
      ...
      sheet.getRange(i+1,1,1,11).setValues([[id,topico,descricao,data,status,idRef,observacao,casoSF,evId,criadoEm,'']]);
      migradas++;
    }

    // Remove colunas extras
    if(sheet.getLastColumn()>11) sheet.deleteColumns(12,sheet.getLastColumn()-11);
...
function salvarAtividade(dados){
  try{
    ...
    sheet.appendRow([id,dados.topico||'Servicos',dados.descricao||'',dados.data||'',dados.status||'Solicitado',dados.idRef||'',dados.observacao||'',dados.casoSF||'',dados.evId||'',new Date().toLocaleString('pt-BR'),dados.descricaoEvento||'']);
    SpreadsheetApp.flush();
    return {success:true,id:id};
  }catch(e){return{success:false,error:e.toString()};}
}
  ...
  try{
    ...
    return rows.filter(function(r){return r[0];}).map(function(r){
      ...
      if(linhaIsNovo && isNovoColunas){
        ...
        return {id:String(r[0]),topico:col2||'Servicos',descricao:String(r[2]||''),data:fmtD(r[3]),status:String(r[4]||'Solicitado'),idRef:String(r[5]||''),observacao:String(r[6]||''),casoSF:String(r[7]||''),evId:String(r[8]||''),criadoEm:String(r[9]||''),descricaoEvento:String(r[10]||'')};
      } else if(linhaIsNovo){
  ...
  try{
    ...
    for(var i=0;i<ids.length;i++){
      if(String(ids[i][0])===String(dados.id)){
        ...
        if(dados.descricaoEvento!==undefined) sheet.getRange(row,11).setValue(dados.descricaoEvento||'');
        SpreadsheetApp.flush();
        return {success:true};
      }
    }
...
function lerTudoPlanilha(){
  ...
  if(erros.length>0){
    Logger.log('lerTudoPlanilha ERROS: '+JSON.stringify(erros));
    resultado._meta.errosDetalhados = erros;
  }
  Logger.log('lerTudoPlanilha: '+resultado._meta.totalColecoes+' coleções. Erros: '+erros.length+(erros.length?' ('+erros.join('; ')+')':''));
  return resultado;
}
...
function escreverLinhaTesteDireto(){
  // VERSAO 5 - testa _ultimaLinhaReal e confirma deploy
  var v='v5-'+new Date().getTime();
  var r={versao:v,success:false};
  try{
    var ss=SpreadsheetApp.getActiveSpreadsheet();
    r.activeSpreadsheet=ss?ss.getName():'NULL';
  }catch(e){r.activeSpreadsheet='ERRO:'+e.message;}
  try{
    var ss2=_getSS();
    r.openById=ss2?ss2.getName():'NULL';
    r.ssId=ss2?ss2.getId():'NULL';
    var sh=ss2.getSheetByName('Despesas');
    if(!sh) sh=ss2.insertSheet('Despesas');
    var ultimaReal=_ultimaLinhaReal(sh);
    r.ultimaLinhaReal=ultimaReal;
    r.getLastRow=sh.getLastRow();
    var destino=ultimaReal+1;
    sh.getRange(destino,1).setValue('TESTE-'+v);
    sh.getRange(destino,2).setValue(new Date().toLocaleString('pt-BR'));
    sh.getRange(destino,3).setValue('AUTO-TEST');
    SpreadsheetApp.flush();
    r.escreveuNaLinha=destino;
    r.success=true;
  }catch(e){r.openByIdErro=e.message;}
  Logger.log('TESTE: '+JSON.stringify(r));
  return r;
}

function testarSalvamento(){
  // Testa se salvarDespesa e salvarManutencao conseguem acessar as abas
  var resultado = {despesas:{}, veiculos:{}};
  try{
    var s1 = getOrCreateSheet('Despesas');
    resultado.despesas.sheet = 'OK ('+s1.getLastRow()+' linhas)';
    var num = getNextDespNum();
    resultado.despesas.nextNum = num;
    resultado.despesas.status = 'OK';
  }catch(e){ resultado.despesas.error = e.toString(); }
  try{
    var s2 = getOrCreateSheet('Gestao de Veiculos');
    resultado.veiculos.sheet = 'OK ('+s2.getLastRow()+' linhas)';
    var num2 = getNextVeiNum();
    resultado.veiculos.nextNum = num2;
    resultado.veiculos.status = 'OK';
  }catch(e){ resultado.veiculos.error = e.toString(); }
...
// ─────────────────────────────────────────────────────────────────────────────
// Corrige o problema de linhas fantasma na aba Despesas:
// Move os registros que foram escritos apos linha 999 para logo apos os dados reais
// e limpa as linhas com apenas formatacao
function recuperarDadosDespesas(){
  try{
    var ss=_getSS();
    var sheet=ss.getSheetByName('Despesas');
    if(!sheet) sheet=ss.insertSheet('Despesas');
    // Limpa a aba e reescreve com dados recuperados
    sheet.clearContents();
    var cabecalho=['No','Data','Comprador','CNPJ','Fornecedor','Descricao','Categoria','Valor','FormaPgto','Foto','SalvoEm','Assinatura','FotoVisual','AssinaturaVisual','Reembolsado'];
    sheet.getRange(1,1,1,15).setValues([cabecalho]);
    var dados=[
      ["DESP-001",46137.0,"Carlos","91.292.987/0015-16","Libraga Brandao Supermercados LTDA","Cafe da manha loja","Alimentação",73.86,"Comprador Pagou","https://drive.google.com/file/d/1SggIhOgvPolt3ApG7YUk_Z4iMhhdEu5N/view",46137.6112037037,"https://drive.google.com/uc?export=view&amp;id=1RWXhCVgvyxZhfUXu-0MYP5lbyNtkRYnR","","","Não"],
      ["DESP-002",46137.0,"Carlos","91.021.238/0001-59","Supermercado Pegoraro","Lanche cafe da manha loja","Alimentação",66.2,"Comprador Pagou","https://drive.google.com/file/d/1NcTew87XlZEnmde69AeduI1IQR44O5Xa/view",46137.612662037034,"https://drive.google.com/uc?export=view&amp;id=11vTCufnmhgnAlCtYTQTjS3bG9DzunSsF","","","Não"],
      ["DESP-003",46139.0,"Christiano","48.136.457/0001-54","Por do sol ","Almoço com o técnico Murilo","Alimentação",74.9,"Comprador Pagou","https://drive.google.com/file/d/1jHID3psrLjL4zbrb93v1C_nXKes38S5y/view",46139.33920138889,"https://drive.google.com/uc?export=view&amp;id=1rlHF1Gx7-ECwQDgwRiNyBj1b8AtEGxvM","","","Sim"],
      ["DESP-004",46139.0,"Christiano","53.691.240/0001-46","Posto Aires","Café ","Alimentação",27.0,"Comprador Pagou","https://drive.google.com/file/d/1D_tNkB2VA6mgUmwCz6iat7urDavhGAmw/view",46139.34171296296,"https://drive.google.com/uc?export=view&amp;id=1rlHF1Gx7-ECwQDgwRiNyBj1b8AtEGxvM","","","Sim"],
      ["DESP-005",46139.0,"Christiano","05.414.384/0001-02","Mini mercado Uniao","Café Lucas e Christino ","Alimentação",22.49,"Comprador Pagou","https://drive.google.com/file/d/1zg44UaNbCyxrS7UbeBLpW2T86ftY9Gty/view",46139.34296296296,"https://drive.google.com/uc?export=view&amp;id=1rlHF1Gx7-ECwQDgwRiNyBj1b8AtEGxvM","","","Sim"],
      ["DESP-006",46141.0,"Carlos","91.021.238/0001-59","Supermercado Pegoraro","Lanche tarde","Alimentação",55.3,"Comprador Pagou","https://drive.google.com/file/d/1GukdVXe2KCRfujk_D-0VPAxed_mAgx2Q/view",46141.716782407406,"https://drive.google.com/uc?export=view&amp;id=1O7Yc912txzuyNbKta2kpZLe6SRIbzvS5","","","Não"],
      ["DESP-007",46141.0,"Carlos","08.771.340/0002-09","Panela Velha Comercio de Alimentos","Cafe da manha em atendimento ","Alimentação",28.0,"Comprador Pagou","https://drive.google.com/file/d/1YicYcP9CowdoeAYBGHmGIy4fFJjoozyy/view",46141.71979166667,"https://drive.google.com/uc?export=view&amp;id=1M9wqjRf1PUYm6xy6MhFFVkgtC2YPS0Iv","","","Não"],
      ["DESP-008",46142.0,"Juliomaq","90.923.061/0001-13","Moro Santos Ltda","Tinta e materiais pintura para grade aradora Tatu","Itens de Serviço e Peças",20.0,"Juliomaq Pagou","https://drive.google.com/file/d/1ih3-66EBneQtGWXAshdIa8edi7slXLP9/view",46142.36662037037,"https://drive.google.com/uc?export=view&amp;id=11It97oR2SsqRt9_ltNDNO49BvU-8S1-Y","","",3082],
      ["DESP-009",46141.0,"Juliomaq","99.024.179/068","Luciano de Arruda Marques Tornearia","Serviços de torno setor peças. Pino Hercules sem graxeira.","Itens de Serviço e Peças",80.0,"Juliomaq Pagará","https://drive.google.com/file/d/1zrzGuDSqag8-dTke_fttw1PCg1ibvqt9/view",46142.373715277776,"https://drive.google.com/uc?export=view&amp;id=1EW_H5EUpCLpNgag4Pvm6j2sgIbxEwsbr","","","Não"],
      ["DESP-010",46142.0,"Juliomaq","91.023.168/0019-05","Cotrijuc Coop Agropec Julio de Castilhos","Ferramenta pos vendas","Itens de Serviço e Peças",31.25,"Juliomaq Pagou","https://drive.google.com/file/d/16JgqwDQdq9fZMnrsH_1DBFtWGaThSk5k/view",46142.37530092592,"https://drive.google.com/uc?export=view&amp;id=1_rIqSgdnvk9CZnCHgVjkJh4aM-gEJXUV","","",3079],
      ["DESP-011",46142.0,"Carlos","00.263.654/0001-08","Restaurante Gomes","2 almoço apos entrega Hercules 15000","Alimentação",102.0,"Comprador Pagou","https://drive.google.com/file/d/1EdevI2WU3_r9fVz5i1M2pcEJRjeeT-1y/view",46142.5634375,"https://drive.google.com/uc?export=view&amp;id=1lMyUEJsApa7KW0gciC22qwHTtq-hX8Aa","","","Não"],
      ["DESP-012",46105.0,"Welinton","37.328.121/0001-77","Intenso Sabor LTDA","Almoço","Alimentação",46.58,"Juliomaq Pagou","https://drive.google.com/file/d/15xPSv5TbtWuQn9qjG2Qd-CsTP6UmR3fN/view",46156.677766203706,"https://drive.google.com/uc?export=view&amp;id=126sVX8SfUvwmKvfQ5wKIjTk7vA27kEZZ","","","Não"],
      ["DESP-013",46106.0,"Welinton","37.328.121/0001-77","Intenso Sabor LTDA","Almoço","Alimentação",36.92,"Juliomaq Pagou","https://drive.google.com/file/d/1Qd5siIL32VAVpHkcxvN8XX_VhRUZYuNM/view",46156.680300925924,"https://drive.google.com/uc?export=view&amp;id=1t_F9HQVTurMCAls7LYRB2B8ZdPBBDlwS","","","Não"],
      ["DESP-014",46157.0,"Carlos",3114,"","teste","Alimentação",1.0,"Juliomaq Pagou",46156.94013888889,"","","","",""],
      ["DESP-015",46158.0,"Carlos","00.000.000/0001-00","TESTE AUTO 8:21","Registro de teste automatico 2026-05-16 8:21","Alimentação",1.99,"Juliomaq Pagou",46158.34861111111,"",2941,"","",""],
      ["DESP-016",46158.0,"Carlos","00.000.000/0001-00","TESTE AUTO 8:26","Registro de teste automatico 2026-05-16 8:26","Alimentação",1.99,"Juliomaq Pagou",46158.35157407408,"",2941,"","",""],
      ["DESP-017",46158.0,"Carlos","00.000.000/0001-00","TESTE AUTO 8:33","Registro de teste automatico 2026-05-16 8:33","Alimentação",1.99,"Juliomaq Pagou",46158.356469907405,"",2941,"","",""]
    ];
    sheet.getRange(2,1,dados.length,15).setValues(dados);
    SpreadsheetApp.flush();
    Logger.log('recuperarDadosDespesas: '+dados.length+' registros restaurados');
    return {success:true,msg:dados.length+' registros restaurados na aba Despesas'};
  }catch(e){
    Logger.log('recuperarDadosDespesas ERRO: '+e);
    return {success:false,error:e.toString()};
  }
}

function corrigirLinhasVeiculos(){
  try{
    var ss=_getSS();
    var sheet=ss.getSheetByName('Gestao de Veiculos');
    ...
    var totalCols=10;
    var deslocadas=[];
    for(var r=500;r<=lastRow;r++){
      var colA=sheet.getRange(r,1).getValue();
      if(colA&&String(colA).trim()!==''&&String(colA).indexOf('TESTE')<0){
        deslocadas.push({row:r,data:sheet.getRange(r,1,1,totalCols).getValues()[0]});
      }
    }
    if(deslocadas.length===0) return {success:true,msg:'Nenhuma linha deslocada.'};
    var proxLinha=_ultimaLinhaReal(sheet)+1;
    for(var i=0;i<deslocadas.length;i++){
      sheet.getRange(proxLinha+i,1,1,totalCols).setValues([deslocadas[i].data]);
      sheet.getRange(deslocadas[i].row,1,1,totalCols).clearContent();
    }
    SpreadsheetApp.flush();
    return {success:true,msg:deslocadas.length+' registros movidos para linha '+proxLinha+'+'};
  }catch(e){return {success:false,error:e.toString()};}
}

function corrigirLinhasDespesas(){
  try{
    var ss=_getSS();
    var sheet=ss.getSheetByName('Despesas');
    if(!sheet) return {success:false,error:'Aba Despesas nao encontrada'};

    var lastRow=sheet.getLastRow();
    var totalCols=15; // A ate O — colunas de dados fixas
    Logger.log('corrigirLinhasDespesas: lastRow='+lastRow);

    // 1. Le linhas deslocadas (>999) com dado real em col A
    var deslocadas=[];
    for(var r=1000;r<=lastRow;r++){
      var colA=sheet.getRange(r,1).getValue();
      if(colA&&String(colA).trim()!==''&&String(colA).indexOf('TESTE')<0){
        var rowData=sheet.getRange(r,1,1,totalCols).getValues()[0];
        deslocadas.push({row:r, data:rowData});
      }
    }
    Logger.log('Linhas deslocadas encontradas: '+deslocadas.length);
    if(deslocadas.length===0){
      return {success:true,msg:'Nenhuma linha deslocada encontrada.'};
    }

    // 2. Acha a proxima linha vazia apos o ultimo dado real (<=999)
    var proxLinha=_ultimaLinhaReal(sheet)+1;
    Logger.log('Proxima linha livre: '+proxLinha);

    // 3. Move cada linha deslocada para a posicao correta
    for(var i=0;i<deslocadas.length;i++){
      var destino=proxLinha+i;
      sheet.getRange(destino,1,1,totalCols).setValues([deslocadas[i].data]);
      // Limpa a linha original
      sheet.getRange(deslocadas[i].row,1,1,totalCols).clearContent();
      Logger.log('Moveu row '+deslocadas[i].row+' -> '+destino+' ('+deslocadas[i].data[0]+')');
    }

    SpreadsheetApp.flush();
    var msg=deslocadas.length+' registros movidos para linhas '+proxLinha+'-'+(proxLinha+deslocadas.length-1);
    Logger.log('corrigirLinhasDespesas OK: '+msg);
    return {success:true,msg:msg};
  }catch(e){
    Logger.log('corrigirLinhasDespesas ERRO: '+e);
    return {success:false,error:e.toString()};
  }
}

function corrigirFormulasPlanilha(){
  try{
    var ss=_getSS();
    var erros=[];
    ...
    // ══════════════════════════════════════════
    // ABA DESPESAS
    // A=No B=Data C=Comprador D=CNPJ E=Fornecedor
    // F=Descricao G=Categoria H=Valor I=FormaPgto
    // J=Foto K=SalvoEm L=Assinatura M=FotoVisual
    // N=AssinaturaVisual O=Reembolsado P=StatusPgto
    // R=Resumo S=Total T=Qtd
    // ══════════════════════════════════════════
    var sd=ss.getSheetByName('Despesas');
    if(sd){
      var lastRowD=_ultimaLinhaReal(sd);

      // Col P: Status Pgto — Google Sheets usa IF (ingles) + separador ;
      sd.getRange('P1').setValue('Status Pgto');
      sd.getRange('P2').clearContent();
      sd.getRange('P2').setFormula(
        '=ARRAYFORMULA(IF(A2:A<>"";IF(I2:I="Comprador Pagou";IF(O2:O="Sim";"Reembolsado";"Aguard. Reembolso");IF(I2:I="Juliomaq Pagou";"Juliomaq Pagou";IF(I2:I="Juliomaq Pagara";"Juliomaq Pagara";"")));""))'
      );

      // Detecta categorias unicas nos dados
      var catSet={};
      if(lastRowD>1){
        sd.getRange(2,7,lastRowD-1,1).getValues().forEach(function(r){
          var c=String(r[0]||'').trim();
          if(c&&!/^\d+$/.test(c)) catSet[c]=true;
        });
      }
      var cats=Object.keys(catSet).sort();

      // Detecta compradores unicos
      var compSet={};
      if(lastRowD>1){
        sd.getRange(2,3,lastRowD-1,1).getValues().forEach(function(r){
          var c=String(r[0]||'').trim();
          if(c&&!/^\d+$/.test(c)) compSet[c]=true;
        });
      }
      var comps=Object.keys(compSet).sort();

      // Limpa resumo R-T linhas 1-40
      sd.getRange(1,18,40,3).clearContent();
      sd.getRange('R1').setValue('Categoria');
      sd.getRange('S1').setValue('Total R$');
      sd.getRange('T1').setValue('Qtd');

      // SUMIF e COUNTIF — nomes em ingles, separador ;
      cats.forEach(function(cat,i){
        var r=i+2;
        sd.getRange('R'+r).setValue(cat);
        sd.getRange('S'+r).setFormula('=SUMIF(G:G;"'+cat+'";H:H)');
        sd.getRange('T'+r).setFormula('=COUNTIF(G:G;"'+cat+'")');
      });
      var tr=cats.length+2;
      sd.getRange('R'+tr).setValue('TOTAL');
      sd.getRange('S'+tr).setFormula('=SUM(H2:H)');
      sd.getRange('T'+tr).setFormula('=COUNTA(A2:A)-1');

      var pr=tr+2;
      sd.getRange('R'+pr).setValue('Por Pagamento');
      ['Comprador Pagou','Juliomaq Pagou','Juliomaq Pagara'].forEach(function(p,i){
        var r=pr+1+i;
        sd.getRange('R'+r).setValue(p);
        sd.getRange('S'+r).setFormula('=SUMIF(I:I;"'+p+'";H:H)');
        sd.getRange('T'+r).setFormula('=COUNTIF(I:I;"'+p+'")');
      });

      var cr=pr+5;
      sd.getRange('R'+cr).setValue('Por Comprador');
      comps.forEach(function(comp,i){
        var r=cr+1+i;
        sd.getRange('R'+r).setValue(comp);
        sd.getRange('S'+r).setFormula('=SUMIF(C:C;"'+comp+'";H:H)');
        sd.getRange('T'+r).setFormula('=COUNTIF(C:C;"'+comp+'")');
      });

      erros.push('Despesas OK: '+cats.length+' cats');
    } else { erros.push('Despesas: nao encontrada'); }

    // ══════════════════════════════════════════
    // ABA GESTAO DE VEICULOS
    // A=No B=Data C=Veiculo D=Tipo E=Responsavel
    // F=Hodometro G=Valor H=Descricao I=Assinatura J=SalvoEm
    // L=Custo/km  N=Veiculo O=Total P=Qtd Q=TipoComum
    // ══════════════════════════════════════════
    var sv=ss.getSheetByName('Gestao de Veiculos');
    if(sv){
      var lastRowV=_ultimaLinhaReal(sv);

      // Col L: Custo/km
      sv.getRange('L1').setValue('Custo/km');
      sv.getRange('L2').clearContent();
      sv.getRange('L2').setFormula('=ARRAYFORMULA(IF(A2:A<>"";IF(F2:F>0;G2:G/F2:F;"");""))');

      // Detecta veiculos unicos
      var veiSet={};
      if(lastRowV>1){
        sv.getRange(2,3,lastRowV-1,1).getValues().forEach(function(r){
          var v=String(r[0]||'').trim();
          if(v&&!/^\d+$/.test(v)) veiSet[v]=true;
        });
      }
      var veiculos=Object.keys(veiSet).sort();

      // Limpa resumo N-Q
      sv.getRange(1,14,veiculos.length+5,4).clearContent();
      sv.getRange('N1').setValue('Veiculo');
      sv.getRange('O1').setValue('Total R$');
      sv.getRange('P1').setValue('Qtd');
      sv.getRange('Q1').setValue('Tipo comum');

      veiculos.forEach(function(vei,i){
        var row=i+2;
        sv.getRange('N'+row).setValue(vei);
        sv.getRange('O'+row).setFormula('=SUMIF(C:C;"'+vei+'";G:G)');
        sv.getRange('P'+row).setFormula('=COUNTIF(C:C;"'+vei+'")');
        sv.getRange('Q'+row).setFormula(
          '=IFERROR(INDEX(D2:D;MATCH(MAX(COUNTIFS(C2:C;"'+vei+'";D2:D;D2:D));COUNTIFS(C2:C;"'+vei+'";D2:D;D2:D);0));"-")'
        );
      });
      var tvr=veiculos.length+2;
      sv.getRange('N'+tvr).setValue('TOTAL');
      sv.getRange('O'+tvr).setFormula('=SUM(G2:G)');
      sv.getRange('P'+tvr).setFormula('=COUNTA(A2:A)-1');

      erros.push('Veiculos OK: '+veiculos.length+' veiculos');
    } else { erros.push('Veiculos: nao encontrada'); }

    SpreadsheetApp.flush();
    var msg=erros.join(' | ');
    Logger.log('corrigirFormulasPlanilha: '+msg);
    return {success:true,msg:msg};
  }catch(e){
    Logger.log('corrigirFormulasPlanilha ERRO: '+e.toString());
    return {success:false,error:e.toString()};
  }
}

// ════════════════════════════════════════════════════════════════
// ── DIAGNÓSTICO E MANUTENÇÃO (sempre por último) ────────────────
// Funções de diagnóstico, limpeza e manutenção de dados.
// Rodar manualmente no Apps Script quando necessário.
// ════════════════════════════════════════════════════════════════
...
function limparCasoSFErrado(){
  // Move dados de tecnicos que estao na col CasoSF (H) para ID_Ref (F)
  // e limpa CasoSF
  try{
    var sheet=getOrCreateSheet('Atividades');
    if(sheet.getLastRow()<2) return {success:false,msg:'Aba vazia'};
    ...
    for(var i=0;i<rows.length;i++){
      var r=rows[i];
      var idRef=String(r[5]||'');
      var casoSF=String(r[7]||'');
      // Se CasoSF tem 'Técnico' mas ID_Ref nao tem, move
      if(casoSF.indexOf('Técnico')>=0){
        var row=i+2;
        if(!idRef||idRef.indexOf('Técnico')<0){
          sheet.getRange(row,6).setValue(casoSF); // move para ID_Ref
        }
        sheet.getRange(row,8).setValue(''); // limpa CasoSF
        corrigidas++;
      }
    }
    SpreadsheetApp.flush();
    var msg='Corrigidas: '+corrigidas+' linhas (técnicos movidos de CasoSF para ID_Ref)';
    Logger.log(msg);
    return {success:true,msg:msg};
  }catch(e){return{success:false,error:e.toString()};}
}
...
function criarOTsRetroativas(){
  try{
    ...
    rows.forEach(function(r){
      ...
      var descricaoEv=String(r[9]||'');
      _criarOTSeNecessario(evId,titulo,cliente,dataISO,tecnicos,descricaoEv);
      criadas++;
    });
    SpreadsheetApp.flush();
...
function removerAbasEventosLeads(){
  try{
    var ss=_getSS();
    var removidas=[];
    ['Eventos','Leads'].forEach(function(nome){
      ...
      if(sh){
        ...
        Logger.log('Aba nao encontrada: '+nome);
      }
    });
    SpreadsheetApp.flush();
    if(removidas.length===0) return {success:true,msg:'Abas ja nao existiam na planilha'};
    return {success:true,msg:'Removidas: '+removidas.join(', ')};
  }catch(e){
    Logger.log('removerAbasEventosLeads ERRO: '+e);
    return {success:false,error:e.toString()};
  }
}
